...
data CIO : (me : proc) -> List proc -> List proc -> Type -> Type where
  Fork : (p : Protocol [c, s] ()) ->
         (Channel s c (protoAs s p) -> CIO s (c :: xs) xs ()) ->
         CIO me xs (s :: xs) (Channel c s (protoAs c p))
  StartServer : (p : Protocol [c, s] ()) ->
                (Channel s c (protoAs s (serverLoop c p)) -> CIO s (c :: xs) (c :: xs) Void) ->
                CIO c xs xs (RChannel t (protoAs c p))
